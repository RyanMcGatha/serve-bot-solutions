...
export function FreeTier({ onSelect }: { onSelect: (active: any) => void }) {
  ...
  return (
    <div className="flex flex-col items-center w-full max-w-7xl mx-auto text-center space-y-6 p-6">
      ...
      <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10 h-full">
        {cards.map((card, index) => (
          <motion.div
            layoutId={`card-${card.title}-${id}`}
            key={index}
            onClick={() => setActive(card)}
            className={`p-6 flex flex-col rounded-xl shadow-lg cursor-pointer transition-all transform hover:scale-105 ${
